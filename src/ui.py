...
class MoveData:
	...
	def buildTrees(self, window, Height=4):
		self.TreeList = {
			'tree': {'Treeview': None, 'EntryText': StringVar(window), 'idlist': []},
	...
	def addSkilsToTree(self, tree, maxslot=3):
		...
		for s_type in Skill_Set:
			...
			for s_name in sorted(temp):
				if s_name in self.MoveList and MoveCodeList[s_name] <= maxslot:
					ids = self.TreeList[tree]['Treeview'].insert(
						id,
						'end',
						self.MoveNames.index(s_name),
						text=s_name,
						values=(MoveCodeList[s_name], Skill_Set[s_type][s_name][0])
					)
		...
		for move in self.MoveList:
			if move not in SkillList and MoveCodeList[move] <= maxslot:
				self.TreeList[tree]['Treeview'].insert(
					id,
					'end',
					self.MoveNames.index(move),
					text=move,
					values=(MoveCodeList[move], "")
				)
...
def SaveASCmd():
	...
	fName = filedialog.asksaveasfilename(
		defaultextension=".bind",
		filetypes=(
			("Amiibo Decripted", "*.bind"),
			("Amiibo Decripted", "*.bind")
		)
	)
	if fName:
...
def Export(var):
	DBName = filedialog.asksaveasfilename(
		defaultextension="." + var + "_db",
		filetypes=(
			("Smash " + var + "_Block", "*." + var + "_db"),
			("Smash " + var + "_Block", "*." + var + "_db")
		)
	)
	if DBName:
...
def ExportDB():
	DBName = filedialog.asksaveasfilename(
		defaultextension=".bind_db",
		filetypes=(
			("Smash DataBlock", "*.bind_db"),
			("Smash DataBlock", "*.bind_db")
		)
	)
	if DBName:
